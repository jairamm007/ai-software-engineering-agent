...
import { createGitHubIssue } from "@/services/github-integration";
import { useMutation, useQueryClient } from "@tanstack/react-query";
import type { GitHubCreateIssueParams } from "@/types/github-integration";
...
export default function GitHubCreateIssueDialog({
  isOpen,
  onClose,
  integrationId,
  owner,
  ...
  const [form, setForm] = useState<GitHubCreateIssueParams>({
    title: "",
    body: "",
    labels: [],
  });
  ...
  const createMutation = useMutation({
    mutationFn: (params: GitHubCreateIssueParams) =>
      createGitHubIssue(integrationId, owner, repo, params),
    onSuccess: () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    createMutation.mutate({ ...form, labels: selectedLabels });
  };
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex justify-end gap-3">
              ...
              <button
                ...
                className="rounded-xl bg-[var(--accent)] px-4 py-2 text-sm font-medium text-white transition-colors hover:opacity-90 disabled:opacity-50"
              >
