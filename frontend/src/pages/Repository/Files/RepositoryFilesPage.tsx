...
export default function RepositoryFilesPage() {
  ...
  useEffect(() => {
    ...
    setSelectedFile(
      files.find((file) => file.id === requestedFileId) ?? files[0]
    );
  }, [repositoryQuery.data?.files, searchParams]);

  if (!id) return null;
  if (repositoryQuery.isLoading) return <DashboardLayout><div className="h-64 animate-pulse rounded-2xl bg-slate-100" /></DashboardLayout>;
    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <div>
          ...
          <p className="mt-2 text-slate-500">Browse the indexed source files in this repository.</p>
        </div>
